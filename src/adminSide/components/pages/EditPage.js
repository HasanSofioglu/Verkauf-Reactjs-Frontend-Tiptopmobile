...
const EditPage = () => {
  ...
  const[phoneModelList, setPhoneList] = useState([])
    useEffect(()=>{
        Axios.get("http://localhost:3001/api/detail/"+phoneId).then((response)=>{
    ...
    const onSubmit = async e => {
      ...
      if(selectedImage!==null){try{
        ...
        const res = await Axios.post('http://localhost:3001/upload/'+phoneId, formData, {
          headers: {
      ...
      try {
          Axios.post("http://localhost:3001/api/update/"+phoneId,{
          phoneName : PhoneName,
